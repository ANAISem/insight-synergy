...
-- *** MANUELLE CREDITS FÜR BESTEHENDE BENUTZER ***
-- Führe dieses Skript aus, um manuell Credits für bestehende Benutzer hinzuzufügen

-- Zuerst müssen wir eine Liste aller Benutzer bekommen, die noch keine Credits haben
WITH users_without_credits AS (
  SELECT u.id 
  FROM auth.users u
  LEFT JOIN user_credits c ON u.id = c.user_id
  WHERE c.user_id IS NULL
)
-- Dann credits für diese Benutzer anlegen
INSERT INTO user_credits (user_id, available_credits, lifetime_credits)
SELECT id, 100, 100
FROM users_without_credits;

-- Und Transaktionen protokollieren
WITH users_without_transactions AS (
  SELECT c.user_id
  FROM user_credits c
  LEFT JOIN credit_transactions t ON c.user_id = t.user_id
  WHERE t.user_id IS NULL
)
INSERT INTO credit_transactions (user_id, amount, transaction_type, description)
SELECT user_id, 100, 'manual_add', 'Manuell hinzugefügte Credits'
FROM users_without_transactions;

-- Stelle auch sicher, dass alle Benutzer Einstellungen haben
WITH users_without_settings AS (
  SELECT u.id 
  FROM auth.users u
