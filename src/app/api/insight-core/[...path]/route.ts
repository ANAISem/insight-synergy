/**
 * Dynamischer API-Proxy für Insight Core
 * 
 * Diese Route ermöglicht die Weiterleitung aller API-Pfade an den Insight Core Backend-Service.
 */
...
async function handleApiRequest(
  ...
  try {
    // Baue den Pfad aus den Segmenten zusammen
    const path = pathSegments.join('/');
    
    // Baue die URL für die Weiterleitung zusammen
    const url = new URL(path, INSIGHT_CORE_API_URL);
    ...
    // Log für Debugging
    console.log(`Proxy-Anfrage an: ${url.toString()}, Methode: ${method}`);
